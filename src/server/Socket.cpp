...
void	Socket::createSocket(void)
{
	...
	int result = getaddrinfo(this->_ip.c_str(), this->_port.c_str(), &hints,
		&this->_serverInfo);
	if (result != 0)
	{
...
void	Socket::bindSocket(void)
{
	...
	if (bind(this->_socketFd, this->_serverInfo->ai_addr,
		this->_serverInfo->ai_addrlen) < 0)
	{
...
int	Socket::acceptConnection(int socketFd)
{
	...
	ss << "New connection from client " << inet_ntop(
			clientAddr.ss_family,
			get_in_addr((struct sockaddr*)&clientAddr),
			remoteIP,
			INET6_ADDRSTRLEN) << " on socket " << clientSocket;
	msg = ss.str();
...
Socket::SocketException::SocketException(const char* msg):
	_msg(msg)
{}
